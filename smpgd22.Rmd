...
In this equation, $\sum_i G_{i,j} y_i$ can be obtained from GWAS summary statistics, while $\sum_i G_{i,j} G_{i,k}$ can be estimated from another dataset.
...
Fortunately, we can consider that the correlation between two genetic variants ( $\sum_i G_{i,j} G_{i,k}$, when $G$ is appropriately scaled) is 0 when they are too distant on the genome. So the matrix $G^T G$, while being e.g. 1M x 1M is actually very sparse (banded).
...
- $r^2$ decreases with polygenicity (proportion of causal variants), because there are more small effects, harder to detect and estimate. Let's denote $M_c$ the number of causal variants.
