...
- [2025] Applied as CRCN at CNRS (CID 51) and INSERM
...
--
    ...
    <span class="small2"> $$PC_\text{ref}^{(l)} = \dfrac{\sum_x {\alpha_l(x)}^m \cdot x}{\sum_x {\alpha_l(x)}^m} ~,$$ </span>
    where $x$ are the PCs for one indiv, and $PC_\text{ref}^{(l)} = \left(PC_1^{(l)},~\dots,~PC_K^{(l)}\right)$
  
--

- Instead of picking L initial $PC_\text{ref}^{(l)}$ at random,    
