...
- We worked on properly validating the inference of $h^2$, $p$, and $\alpha$ (and their CIs) using simulations.

- We also showed that LDpred2-auto provide calibrated per-variant probabilities of being causal.

- We derived a formula to infer the out-of-sample predictive performance $r^2$ directly from the Gibbs sampler.
...
Soon to be published in AJHG. Stay tuned `r icons::fontawesome$brands$twitter` @privefl.
...
- For now, we have recommended to use a set of 1,054,330 HapMap3 variants, because they provide a good coverage of the genome and are generally well imputed and available in most studies

- We now provide an extended set with 37% more variants, which we called HapMap3+, designed to maximize tagging of 11.5M common variants in diverse genetic ancestries.

- Using this new set of variants, we capture 12% more heritability and obtain 6% more predictive performance, on average, in UK Biobank analyses.
...
#### again, in case you forgot already `r emo::ji("smile")`
