...
You can then **run any predictive model** that is scalable to the size of the data, e.g. in `r ICON_R_PROJECT` package {bigstatsr}, there is a very fast implementation of penalized regression with automatic choice of hyper-parameters [[bit.ly/plr-bigstatsr](https://bit.ly/plr-bigstatsr)]
...
- using R package bigsnpr or software PRSice-2 to build C+T scores 

--

- approximating a penalized regression (lassosum, lassosum2): the coordinate descent algorithm can be approximated using summary statistics only 

    - marginal GWAS summary statistics
    - reference for correlation structure between variants

--

- using Bayesian methods (LDpred2, PRS-CS, SBayesR, ...), which can infer model parameters and does not require any tuning set for hyper-parameters
...
- Predictive power $r^2$ is bounded by the (SNP) heritability $h^2$ captured by the set of variants used.
