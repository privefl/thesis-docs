...
You can then **use any predictive model** that is scalable to the size of the data, e.g. in `r ICON_R_PROJECT` package {bigstatsr}, there is a very fast implementation of penalized regression with automatic choice of hyper-parameters [[bit.ly/plr-bigstatsr](https://bit.ly/plr-bigstatsr)]
...
- approximating a penalized regression (lassosum, lassosum2) using summary statistics only, i.e.

    - marginal GWAS summary statistics $\hat{\beta}$
    - reference for correlation structure between variants $R$    
    $\Longrightarrow \hat{\beta} = R \beta$, we need to estimate $\beta$
