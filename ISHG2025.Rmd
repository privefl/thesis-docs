...
# renderthis::to_pdf("ISHG2025.Rmd", partial_slides = TRUE, complex_slides = TRUE, delay = 2)
...
$$\large L(Q, F) = \sum_i \sum_j \left\{ G_{i,j} \log\left[\sum_l Q_{i,l} F_{l,j}\right] + (2 - G_{i,j}) \log\left[1 - \sum_l Q_{i,l} F_{l,j}\right] \right\}$$
...
$$\large \min_{\forall l,~Q_{i,l} \ge 0 \\ \sum_l Q_{i,l}=1} ~~ \sum_{k=1}^K \left( PC_{i,k} - \sum_{l=1}^L Q_{i,l} PC_{l,k}^{\text{ref}} \right)^2$$
...
### Admixture coefficients $\Large Q$ using L=5 reference populations
...
### Projected reference allele frequencies ($\Large 2F \cdot V$) from ADMIXTURE

```{r, out.width="80%"}
knitr::include_graphics("figures/1000G-admixture-5refs.png")
```

#### $\large \Rightarrow$ highly similar to what I get with my method

#### $\large \Rightarrow$ supports using $K$ PCs only for $L=K+1$ reference populations

---

### Admixture coefficients $\Large Q$ using L=9 reference populations
...
### Admixture coefficients $\Large Q$ using L=18 reference populations
...
$\large \Rightarrow$ may be used for cell type deconvolution of methylation data
...
Presentation available at [bit.ly/privefl_ISHG2025](https://bit.ly/privefl_ISHG2025)
]
