...
- in practice, we don't use causal variants 

- we use tagging variants, highly correlated with the causal variants    
*in the training population*

- but the correlation between tagging and causal variants is different (usually much lower) in other populations
...
- compact sparse format (divide by 2)

- storing correlations with two bytes only (divide by 4)
...
- adapt methods to use very sparse *inverse* correlation matrices 
...
But we need to use many variants (WP1) to keep annotations useful

```{r, out.width="70%"}
knitr::include_graphics("figures/pb-anno.png")
```
...
```{r, out.width="100%"}
knitr::include_graphics("figures/multiancestry-finemapping.jpg")
```
...
- there are lots of problems with GWAS summary statistics (input data)
...
- Background in Statistics and Informatics 

- Already developed many state-of-the-art methods (recognized internationally)

- I published 9 first-author, 2 sole-author and 4 co-senior-author papers

- I already found many collaborators for this projects
