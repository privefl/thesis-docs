...
$$\large G \approx Q \cdot 2F$$

- $\large Q$ are the admixture proportions (for each sample $\large i$ and reference $\large l$)

- $\large F$ are the allele frequencies (for each each reference $\large l$ and variant $\large j$)
...
<span class="small3"> $$\large L(Q, F) = \sum_i \sum_j \left\{ G_{i,j} \log\left[\sum_l Q_{i,l} F_{l,j}\right] + (2 - G_{i,j}) \log\left[1 - \sum_l Q_{i,l} F_{l,j}\right] \right\}$$ </span>

with constraints: $\large ~0 \le F_{l,j} \le 1~$ and $\large ~Q_{i,l} \ge 0~$ and $\large ~\sum_l Q_{i,l} = 1$
...
- each $\large Q_{i,.}$ independently, with $\large F$ fixed

- each $\large F_{.,j}$ independently, with $\large Q$ fixed
...
$$\large G \cdot V \approx Q \cdot 2F \cdot V$$
where $\large V$ are the PC loadings of $\large G$

$$\large \Rightarrow~ PC \approx Q \cdot PC^\text{ref}$$
--
...
### Estimating admixture coefficients $\Large Q_{i,.}$ with $\Large PC^\text{ref}$ fixed

<span class="small3"> $$\large \min_{\forall l,~Q_{i,l} \ge 0 \\ \sum_l Q_{i,l}=1} ~~ \sum_{k=1}^K \left( PC_{i,k} - \sum_{l=1}^L Q_{i,l} PC_{l,k}^{\text{ref}} \right)^2$$ </span>
...
### Estimating admixture coefficients $\Large PC_{l,.}^{\text{ref}}$ with $\Large Q$ fixed

<br>

$$\large PC_{l,.}^{\text{ref}} = \dfrac{\sum_i {Q_{i,l}}^m \cdot PC_{i,.}}{\sum_i {Q_{i,l}}^m}$$

<br>

- this simple formula is used in e.g. fuzzy K-means

- this is also related to archetypal analysis: $\large PC^{\text{ref}} = W^T \cdot PC$    
(references are weighted combinations of existing samples)

    $\Rightarrow$ Reference allele frequencies: $\large 2F = W^T \cdot G$
...
- estimating reference positions $\large PC_{l,.}^{\text{ref}}$, with $\large Q$ fixed
...
A naive approach would pick L initial $\large PC_{l,.}^{\text{ref}}$ at random.
...
- I've also looked at using $\large Q$ to do the subsampling
