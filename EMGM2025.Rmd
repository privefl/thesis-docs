...
### Admixture model and method

$$G \approx Q \cdot 2F$$

- $Q$ are the ancestry proportions (for each indiv $i$ and ref $l$)

- $F$ are the allele frequencies (for each each ref $l$ and variant $j$)

--

***

It uses Maximum Likelihood Estimation of

<span class="small3"> $$L(Q, F) = \sum_i \sum_j \left\{ G_{i,j} \log\left[\sum_l Q_{i,l} F_{l,j}\right] + (2 - G_{i,j}) \log\left[\sum_l Q_{i,l} (1 - F_{l,j})\right] \right\}$$ </span>
...
- curated 18 reference groups within the UK Biobank

- provide allele frequencies and PC loadings

- work on individual-level data or GWAS allele frequencies only

- more power to do the minimization in the PCA space
...
(references are weighted combinations of existing individuals)
...
- estimating admixture coefficients $Q_{i,.}$ (for each individual $i$)

- estimating reference positions $PC_{l,.}^{\text{ref}}$ (for each ref $l$) </span>
...
### A complex admixture simulation
