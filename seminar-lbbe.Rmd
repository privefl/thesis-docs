...
- Iterate between 
    ...
    - estimating ancestry coefficients with
    <span class="small2"> $$\min_{\alpha_l \ge 0 \\ \sum_{\alpha_l}=1} ~~ \sum_{k=1}^K \left( PC_k^{(0)} - \sum_{l=1}^L \alpha_l PC_k^{(l)} \right)^2$$ </span>
...
I sequentially add one more PC and one more reference    
(with good starting values)

---

### One prototyped method (with added fuzziness = 1.2)
...
One prototyped method (without any added fuzziness)

<br>

```{r, out.width="100%"}
knitr::include_graphics("figures/clust_myalgo_nofuzz.png")
```
